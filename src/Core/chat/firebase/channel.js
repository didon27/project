...
export const subscribeThreadSnapshot = (channel, callback, limit = 20) => {
...
export const getThreadSnapshot = (channel, callback, prevNode, limit = 20) => {
  // console.log(
  //   'getThreadSnapshot -> prevNode',
  //   prevNode ? prevNode['_data'].created : 'Null',
  // );
  let query = channelsRef
    .doc(channel.id)
    .collection('thread')
  ...
  if (prevNode) {
    query = query.startAfter(prevNode);
  }
  query = query.limit(limit);

  return query.get().then(callback);
};

export const subscribeThreadSnapshotTest = (channel, callback) => {
  return channelsRef
    .doc(channel.id)
    .collection('thread')
    .orderBy('created', 'desc')
...
export const updateChannelLanguages = (channelId, userId, from, to) => {
  const lan = { language: { [userId]: { from: from, to: to } } };
  channelsRef.doc(channelId).set(
    {
      ...lan,
    },
    { merge: true },
