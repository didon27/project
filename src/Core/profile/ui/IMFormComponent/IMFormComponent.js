...
function IMFormComponent(props) {
  ...
  const renderContactField = (formTextField, index, totalLen) => {
    return (
      <TouchableOpacity
        onPress={() => onContactPress(formTextField)}
        activeOpacity={0.7}>
        <View
          style={[
            styles.settingsTypeContainer,
  ...
  const renderLinkField = linkField => {
    return (
      <TouchableOpacity
        onPress={() => Linking.openURL(ChatConfig.tosLinkPP)}
        style={[styles.settingsTypeContainer, styles.appSettingsTypeContainer]}>
        <Text style={styles.settingsType}>{linkField.displayName}</Text>
      </TouchableOpacity>
    );
  };
  ...
  const renderField = (formField, index, totalLen) => {
    ...
    if (type === 'contact') {
      return renderContactField(formField);
    }
    if (type == 'button') {
    ...
    if (type == 'link') {
      return renderLinkField(formField);
    }
    return null;
  };
